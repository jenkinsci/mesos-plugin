...
public class MesosImpl extends Mesos {
  ...
  public boolean isSchedulerRunning() {
    lock();
    try {
      boolean status = (scheduler != null && scheduler.isRunning());
      if (status) {
  ...
  public void startJenkinsSlave(SlaveRequest request, SlaveResult result) {
    ...
    try {
      if (scheduler != null) {
        LOGGER.fine("Starting Jenkins Slave with request: " + request);
  ...
  public void stopJenkinsSlave(String name) {
    ...
    try {
      if (scheduler != null) {
        LOGGER.fine("Stopping Jenkins Slave with name: " + name);
